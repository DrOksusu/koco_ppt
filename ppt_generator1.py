...
def create_first_slide(prs, df, df_raw, id_photo_path):
   ...
   # 5️⃣ 첫 번째 열과 비교하여 일치하는 경우 두 번째 열 업데이트
    for i in range(0, len(table.rows)):  # 첫 번째 행(헤더 제외)
        ...
        if standardized_key in ceph_standardized:  # 표준화된 키가 ceph 딕셔너리에 존재하면
            table.cell(i, 1).text = str(ceph_standardized[standardized_key])  # 두 번째 열에 값 입력
