...
function drawYellowCircles(ctx, points, scaleX, scaleY, p_perp) {
  ...
  let p1_x = points[3][0],
    p1_y = points[3][1];
  let a = points[2][0],
    b = points[2][1]; // hinge point
  let c = points[3][0],
    d = points[3][1]; // lower incisor tip
  ...
  let x1_red = points[3][0],
    y1_red = points[3][1];
  let x2_red = points[4][0],
    y2_red = points[4][1];
    ...
    (p) =>
      !(
        (
          ...
          Math.abs(p[1] - p1_y) <= Math.abs(p1_y * 0.02)
        ) // Y 좌표 0.1% 오차 범위
      )
  );
...
function drawArrowFromYellowCircle(
  ...
  if (isNaN(HGI) || isNaN(VGI)) {
    console.warn(
      "❌ HGI 또는 VGI 값이 숫자가 아닙니다. 기본값(0)으로 처리합니다."
    );
    HGI = 0;
    VGI = 0;
  }
